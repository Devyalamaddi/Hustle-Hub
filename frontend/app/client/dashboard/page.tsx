...
export default function ClientDashboard() {
  ...
  const fetchJobs = async () => {
    try {
      const response = await fetch("/api/client-api/jobs", {
        headers: {
  ...
  const fetchGigsForJob = async (jobId: string) => {
    try {
      const response = await fetch(`/api/client-api/jobs/${jobId}/gigs`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      })
  ...
  const confirmFreelancer = async (jobId: string, freelancerId: string) => {
    try {
      const response = await fetch(`/api/client-api/jobs/${jobId}/confirm-gig/${freelancerId}`, {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
